...
const CustomerDynamicBidding = ({ orderId, lockoutTimeInMinutes, setRenderComponant, setDisplayBidding }) => {
  ...
  const columns = [
    ...
    { field: "vendorRating", headerName: "Vendor Rating", flex: 0.5, renderCell : RenderRating },
    ...
    { field: "selectVendor", headerName: "Select Vendor", flex : 0.5, renderCell : (params) => {
            const handleClick = (e) => {
                ...
                axios.post("http://localhost:8080/order/finalize", finalizeDTO).then(res=> { if(res.status === 200){ setDisplayBidding(false); setRenderComponant("pending") }}).catch(err=> console.log(err));
            }
            return <Button onClick={handleClick} sx={{position : "relative", zIndex : 2}}>Start Order</Button>
    }}
  ];
