...
function ActiveOrderTable({ customerId, setRenderComponant }){
    ...
    const func = ()=> {
        axios.get("http://localhost:8080/order/active/"+customerId).then(res=> setActiveOrders(res.data)).catch(err=>console.log(err));
    }
            ...
            <tbody>
                {activeOrders.map(order => {
                    return  <tr>
                                ...
                                <td></td>
                                <td></td>
                                <td> <Timer minutes={order.lockoutTimeInMinutes}/></td>
                                <td><CustomerBiddingModal order={order} setRenderComponant={setRenderComponant}/></td>
                            </tr>
